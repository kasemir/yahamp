				...
				for(Object existingObject : list) {
					...
					if (EcoreUtil.equals((EObject)existingEObject, (EObject)element)) {
						...
						break;
	                       ...
	                       // Remember the existing element's parent (if any)
					    MElementContainer<MUIElement> curParent = existingEObject instanceof MUIElement ? ((MUIElement)existingEObject).getParent() : null;

					    EcoreUtil.replace((EObject)existingEObject, (EObject)element);

					    // Replace the parent's selected element with the replacement (if necessary)
					    if (curParent != null && curParent.getSelectedElement() == existingEObject)
					        curParent.setSelectedElement((MUIElement) element);
